...
import tak.bo.irc.ircclient.ConnectionService;
import tak.bo.irc.ircclient.ircbotsoftware.IRCClient;
import tak.bo.irc.ircclient.ircbotsoftware.NickAlreadyInUseException;

public class ConnectionWrapper extends IRCClient
{
     ...
     * @param serverId
     */
    public ConnectionWrapper(ConnectionService service, int serverId)
    {
    ...
    public void onConnect()
    {
        ...
        server.getRoom(ServerInfo.DEFAULT_NAME).addMessage(message);

        Message infoMessage = new Message(service.getString(R.string.message_now_login));
        infoMessage.setColor(Message.COLOR_GREY);
        server.getRoom(ServerInfo.DEFAULT_NAME).addMessage(infoMessage);

        Intent intent = Broadcast.createRoomIntent(
    ...
    public void onRegister()
    {
        ...
        for (String command : server.getConnectCommands()) {
            parser.parse(command, server, server.getRoom(ServerInfo.DEFAULT_NAME), service);
        }
    ...
    protected void onAction(String sender, String login, String hostname, String target, String action)
    {
        ...
        conversation = server.getRoom(queryNick);
        ...
        if (mentioned || target.equals(this.getNick()) || conversation.shouldAlwaysNotify()) {
            if (conversation.getStatus() != Room.STATUS_SELECTED || !server.getIsForeground()) {
        ...
        if (mentioned) {
            ...
            conversation.setStatus(Room.STATUS_HIGHLIGHT);
        }
    }
    ...
    protected void onDeop(String target, String sourceNick, String sourceLogin, String sourceHostname, String recipient)
    {
        ...
        server.getRoom(target).addMessage(message);
    ...
    protected void onInvite(String targetNick, String sourceNick, String sourceLogin, String sourceHostname, String target)
    {
        if (targetNick.equals(this.getNick())) {
            ...
            server.getRoom(server.getSelectedRoom()).addMessage(message);

            Intent intent = Broadcast.createRoomIntent(
                Broadcast.CONVERSATION_MESSAGE,
                server.getId(),
                server.getSelectedRoom()
            );
            service.sendBroadcast(intent);
        } else {
            // Someone is invited
    ...
    {
        if (sender.equalsIgnoreCase(getNick()) && server.getRoom(target) == null) {
            // We joined a new channel
            Room conversation = new Channel(target);
            conversation.setHistorySize(service.getSettings().getHistorySize());
            server.addRoom(conversation);
    ...
    protected void onMessage(String target, String sender, String login, String hostname, String text)
    {
        ...
        Room conversation = server.getRoom(target);
    ...
    protected void onNotice(String sourceNick, String sourceLogin, String sourceHostname, String target, String notice)
    {
        ...
        if (service.getSettings().showNoticeInServerWindow()) {
            conversation = server.getRoom(ServerInfo.DEFAULT_NAME);
        } else {
            conversation = server.getRoom(server.getSelectedRoom());
    ...
    protected void onPrivateMessage(String sender, String login, String hostname, String target, String text)
    {
        ...
        Room conversation = server.getRoom(queryNick);
    ...
    protected void onQuit(String sourceNick, String sourceLogin, String sourceHostname, String reason)
    {
        ...
        if (service.getSettings().showJoinPartAndQuit()) {
            ...
            Room conversation = server.getRoom(sourceNick);
    ...
    public void onTopic(String target, String topic, String setBy, long date, boolean changed)
    {
        ...
        ((Channel) server.getRoom(target)).setTopic(topic);
        ...
        intent = Broadcast.createRoomIntent(
    ...
    protected void onRemoveChannelKey(String target, String sourceNick, String sourceLogin, String sourceHostname, String key)
    {
        ...
        service.sendBroadcast(
            Broadcast.createRoomIntent(Broadcast.CONVERSATION_MESSAGE, server.getId(), target)
        );
    }
    ...
    protected void onServerResponse(int code, String response)
    {
        ...
        if (code == 376 && ignoreMOTD) {
            ...
            server.getRoom(ServerInfo.DEFAULT_NAME).addMessage(motdMessage);
            ignoreMOTD = false;
            return;
        }
    ...
    public void onDisconnect()
    {
        ...
        Collection<Room> conversations = server.getRooms();

        for (Room conversation : conversations) {
            Message message = new Message(service.getString(R.string.message_disconnected));
            message.setIcon(R.drawable.error);
            message.setColor(Message.COLOR_RED);
            server.getRoom(conversation.getName()).addMessage(message);

            Intent cIntent = Broadcast.createRoomIntent(
                Broadcast.CONVERSATION_MESSAGE,
                server.getId(),
                conversation.getName()
            );
